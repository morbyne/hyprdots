...
if autoload -U is-at-least && is-at-least 5.7 && [[ $COLORTERM == (24bit|truecolor) || ${terminfo[colors]} -eq 16777216 ]]; then
    EVERGARDEN_SPRING_PURE_POWER_TRUE_COLOR=true
else
    EVERGARDEN_SPRING_PURE_POWER_TRUE_COLOR=false
fi
...
() {
  ...
  if [[ "$EVERGARDEN_SPRING_PURE_POWER_TRUE_COLOR" == "true" ]]; then
    local ok="%F{#cbe3b3}${p}%f"
    local err="%F{#f57f82}${p}%f"
  else
    local ok="%F{$(_pp_s 010 151)}${p}%f"
  ...
  if (( ${PURE_POWER_USE_P10K_EXTENSIONS:-1} )); then
    ...
    if [[ "$EVERGARDEN_SPRING_PURE_POWER_TRUE_COLOR" == "true" ]]; then
      typeset -g POWERLEVEL9K_RULER_FOREGROUND='#58686d'
    else
      typeset -g POWERLEVEL9K_RULER_FOREGROUND=$(_pp_c 8 242)
    fi
  else
  ...
  if [[ "$EVERGARDEN_SPRING_PURE_POWER_TRUE_COLOR" == "true" ]]; then
    typeset -g POWERLEVEL9K_DIR_WRITABLE_FORBIDDEN_VISUAL_IDENTIFIER_COLOR='#f5d098'
  else
    typeset -g POWERLEVEL9K_DIR_WRITABLE_FORBIDDEN_VISUAL_IDENTIFIER_COLOR=$(_pp_c 003 222)
  fi
  ...
  if [[ "$EVERGARDEN_SPRING_PURE_POWER_TRUE_COLOR" == "true" ]]; then
    typeset -g POWERLEVEL9K_DIR_{ETC,DEFAULT}_FOREGROUND='#b3e3ca'
    typeset -g POWERLEVEL9K_DIR_{HOME,HOME_SUBFOLDER}_FOREGROUND='#cbe3b3'
    typeset -g POWERLEVEL9K_DIR_NOT_WRITABLE_FOREGROUND='#f57f82'
  else
    typeset -g POWERLEVEL9K_DIR_{ETC,DEFAULT}_FOREGROUND=$(_pp_c 014 158)
    typeset -g POWERLEVEL9K_DIR_{HOME,HOME_SUBFOLDER}_FOREGROUND=$(_pp_c 010 151)
    typeset -g POWERLEVEL9K_DIR_NOT_WRITABLE_FOREGROUND=$(_pp_c 001 210)
  fi
  ...
  if [[ "$EVERGARDEN_SPRING_PURE_POWER_TRUE_COLOR" == "true" ]]; then
    typeset -g POWERLEVEL9K_VCS_CLEAN_FOREGROUND='#b3e3ca'
    typeset -g POWERLEVEL9K_VCS_UNTRACKED_FOREGROUND='#f5d098'
    typeset -g POWERLEVEL9K_VCS_MODIFIED_FOREGROUND='#f3c0e5'
    typeset -g POWERLEVEL9K_VCS_LOADING_FOREGROUND='#839e9a'
  else
    typeset -g POWERLEVEL9K_VCS_CLEAN_FOREGROUND=$(_pp_c 014 158)
    typeset -g POWERLEVEL9K_VCS_UNTRACKED_FOREGROUND=$(_pp_c 011 222)
    typeset -g POWERLEVEL9K_VCS_MODIFIED_FOREGROUND=$(_pp_c 013 219)
    typeset -g POWERLEVEL9K_VCS_LOADING_FOREGROUND=$(_pp_c 007 109)
  fi
  ...
  if [[ "$EVERGARDEN_SPRING_PURE_POWER_TRUE_COLOR" == "true" ]]; then
    typeset -g POWERLEVEL9K_STATUS_ERROR_FOREGROUND='#f57f82'
  else
  ...
  if [[ "$EVERGARDEN_SPRING_PURE_POWER_TRUE_COLOR" == "true" ]]; then
    typeset -g POWERLEVEL9K_COMMAND_EXECUTION_TIME_FOREGROUND='#839e9a'
  else
    typeset -g POWERLEVEL9K_COMMAND_EXECUTION_TIME_FOREGROUND=$(_pp_c 008 109)
  fi
  ...
  if [[ "$EVERGARDEN_SPRING_PURE_POWER_TRUE_COLOR" == "true" ]]; then
    typeset -g POWERLEVEL9K_CONTEXT_{DEFAULT,REMOTE_SUDO,REMOTE,SUDO}_FOREGROUND='#f5d098'
    typeset -g POWERLEVEL9K_CONTEXT_ROOT_FOREGROUND='#f57f82'
  else
    typeset -g POWERLEVEL9K_CONTEXT_{DEFAULT,REMOTE_SUDO,REMOTE,SUDO}_FOREGROUND=$(_pp_c 003 222)
    typeset -g POWERLEVEL9K_CONTEXT_ROOT_FOREGROUND=$(_pp_c 001 210)
  fi
